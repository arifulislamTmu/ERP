   ...
   <section class="content">
      <div class="row d-flex justify-content-center mt-4">
       <div class="col-lg-10 ">
         <?php
          ...
          <strong>Warning! </strong> Data deleted success ....!
            ...
            if(isset($_POST['create_btn'])){
                ...
                $select ="SELECT * FROM tbl_manage_expance WHERE date='$date' AND employe_id='$employe_id' AND expance_head='$expance_head' AND amount='$amount' AND pay_method='$pay_method' AND note='$note' AND details='$details'"; 
                $run = mysqli_query($con,$select);
                if(mysqli_num_rows($run)>0){
                  echo $msg2;
                }else{

                $insert ="INSERT INTO tbl_manage_expance(date,expance_head,employe_id,amount,pay_method,note,details)
                VALUES('$date','$expance_head','$employe_id','$amount','$pay_method','$note','$details')";
        ...
        if(isset($_GET['employe_delete'])){
          $employe_delete = $_GET['employe_delete'];
    
          $delete = "DELETE FROM tbl_manage_expance WHERE id ='$employe_delete'";
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">Add Expenditure Employe</h3>
            ...
            <div class="col-lg-3">
                <div class="form-group">
                    ...
                    <?php
                        ...
                        <option value="<?php echo $result['employe_id']?>"><?php echo $result['empole_name']?></option>
                    <?php   } ?>
                    </select>
                </div>
            </div>
                  ...
                  <form action="" method="POST">
                     <div class="row">
                           ...
                           <div class="total">
                            <h5>
                              <?php
                                 ...
                                 INNER JOIN tbl_employe ON tbl_manage_expance.employe_id = tbl_employe.employe_id
                                  INNER JOIN tbl_expance_head ON tbl_manage_expance.expance_head =tbl_expance_head.expnc_id ORDER BY id DESC"; 
                                   $run = mysqli_query($con,$select);
                                 while($result = mysqli_fetch_array($run)){ ?>
                                  Total Expance : <strong><?php echo $result['sum'];?></strong> 
                               <?php  }?>
                            </h5>
                           </div>
                         <?php 
                         if(isset($_POST['search'])){
                              ...
                              $selct_amont = "SELECT SUM(amount) AS sum from tbl_manage_expance 
                                              WHERE date  BETWEEN '$form_date' AND '$to_date' AND employe_id >0";
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    <?php
                    $count = 1;
                      $select = "SELECT * FROM tbl_manage_expance 
                      INNER JOIN tbl_employe ON tbl_manage_expance.employe_id = tbl_employe.employe_id
                      INNER JOIN tbl_expance_head ON tbl_manage_expance.expance_head = tbl_expance_head.expnc_id ORDER BY id DESC"; 
                        $run = mysqli_query($con,$select);
                      while($result = mysqli_fetch_array($run)){ ?>
                    <tr>
                      ...
                      <td> <?php if($role_name =='0'){ ?> <a style="font-size:20px; padding:10px;"href="employe_expace_list_edit.php?employe_id_edit=<?php echo $result['id'];?>"><i class="fas fa-edit"></i></a>  <a  onclick="return confirm('You are Sure..?')" style="font-size:20px; padding:10px;"href="?employe_delete=<?php echo $result['id'];?>"><i class="far fa-trash-alt"></i></a> <?php }?> </td>
                    </tr>
                  <?php } ?>
                </table>
              </div>
